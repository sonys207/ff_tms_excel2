...

{
    ...
    public function excelandemail(Request $Request)
    {  
        ...
        $header = array(
          ...
          $styles1 = array( 'font'=>'Arial', 'font-size'=>10, 'font-style'=>'bold', 'fill'=>'#eee', 'halign'=>'center', 'valign'=>'center', 'border'=>'left,right,top,bottom', 'border-style'=>'thin', 'wrap_text'=>true, 'freeze_rows'=>1, 'widths'=>[12,10,7,11,12,8,8,8,8,8,8,8,12,8]);
          $styles2 = array( 'font'=>'Arial', 'font-size'=>10, 'halign'=>'center','valign'=>'center', 'border'=>'left,right,top,bottom', 'border-style'=>'thin', 'height'=>22);
          //需要数组实现
          $data = array(
              array('Sam',872711,'UV','18:00-20:00','M5J 2Z3','B 32',3,1,'',1,'',5,'',''),
              array('Sam',872712,'UV','18:00-20:00','M5J 2Z3','B 45',1,2,'',1,'',4,'',''),
              array('Sam',872713,'UV','18:00-20:00','M5J 2Z3','B 55',3,2,'',1,'',6,'',''),
              array('','','','','','','','','','','','','',''),
              array('Ginny',872713,'UV','18:00-20:00','M5J 2Z3','B 55',5,2,'',2,'',9,'',''),
          );

          $writer = new XLSXWriter();
          $writer->setTitle('fulfillment order list');
          $writer->setSubject('fulfillment order list');
          $writer->setAuthor('TMS');
          $writer->setCompany('TNT');
          $writer->setDescription('fulfillment order list');
          $writer->writeSheetHeader('fulfillment order list', $header, $styles1);
          foreach($data as $row)
              $writer->writeSheetRow('fulfillment order list', $row, $styles2 );
        //合并单元格需要获取每个司机的订单数量，然后通过数组循环实现
          $writer->markMergedCell('fulfillment order list', $start_row=1, $start_col=0, $end_row=3, $end_col=0); 
          $writer->markMergedCell('fulfillment order list', $start_row=1, $start_col=10, $end_row=3, $end_col=10);  
          $writer->markMergedCell('fulfillment order list', $start_row=1, $start_col=12, $end_row=3, $end_col=12);  
          $writer->markMergedCell('fulfillment order list', $start_row=4, $start_col=0, $end_row=4, $end_col=13);   
          $writer->writeToFile('C:\TMS-Lumen\tnt-report.xlsx');
          sleep(2);
          $email = new Mail(); 
          $email->setFrom("jichen.liu86@gmail.com", "test User");
          $email->setSubject("Sending with SendGrid is Fun");
          $email->addTo("jichen.liu85@gmail.com", "sendgrid");
          $email->addTo("tony.liu@cmeow.com", "sendgrid1");
          $email->addCc("jichen.liu87@gmail.com", "sendgrid2");
          $email->addContent(
              "text/html", "<strong>2and easy to do anywhere, even with PHP</strong>"
          );
  
          $att1 = new Attachment();
          $att1->setContent(file_get_contents('C:\TMS-Lumen\tnt-report.xlsx'));
          $att1->setType("application/octet-stream");
          $att1->setFilename(basename('C:\TMS-Lumen\tnt-report.xlsx'));
    ...
    public function xlsgenerate1(Request $Request)
    {
        $data = array(
        ...
        $writer->writeToFile('C:\TMS-Lumen\output1.xlsx');
        var_dump('123');
    } 


    public function xlsgenerate2(Request $Request)
    {
        $header = array(
            'created'=>'date',
            'product_id'=>'integer',
          ...
          $data = array(
              array('2015-01-01',873.9,15,'44.00','misc','=D2*0.05'),
              array('2015-01-12',324,2800,'88.00','none','=D3*0.05'),
          );
          ...
          $writer->writeToFile('C:\TMS-Lumen\output2.xlsx');
    } 

    public function xlsfulfillmentorderlist(Request $Request)
    {
        $header = array(
            'Driver'=>'string',
            'Order No.'=>'integer',
